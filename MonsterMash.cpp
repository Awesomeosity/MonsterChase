...
unsigned int GetMonsterCount(const unsigned int maxMonsters)
{
	...
	while (true)
	{
		...
		if (std::cin.fail() || monsterCount < 0 || monsterCount > maxMonsters)
		{
			std::cin.clear();
			std::cin.ignore(INT_MAX, '\n');
			std::cout << "Invalid input. Please try entering an positive integer, or enter in a number less than " << maxMonsters << ".\n";
		}
		else
		{
			std::cin.ignore(INT_MAX, '\n');
...
void MonsterPrint(Monster* monsters, const unsigned int maxMonsters)
{
...
void MoveMonsters(const int maxMonsters, Monster* monsters, Player* player)
{
...
void SpawnMonsters(Monster* monsters, const int maxMonsters, const int playX, const int playY)
{
	for (int i = 0; i < maxMonsters; i++)
	{
...
bool CheckPlayer(Player* player, Monster* monsters, const int maxMonsters)
{
...
void GameLoop(Monster* monsters, Player* player, const int maxMonsters, const int playX, const int playY)
{
	//Initial startup coverage.
...
Monster* MonsterCreateLoop(int playX, int playY, int* const count, const unsigned int maxMonsters)
{
	unsigned int i = 0;
	unsigned int counts = GetMonsterCount(maxMonsters);
	*count = counts;
	std::cout << counts << " is the initial amount of monsters.\n";
...
int main()
{
	...
	static const unsigned int maxMonsters = 16;
	int* const count = new int;
	std::cout << "Monster Mash by Kevin Le (u0916211)\n";
	Monster* monsters = MonsterCreateLoop(playX, playY, count, maxMonsters);
