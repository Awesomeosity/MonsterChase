...
void CollisionUnitTest()
{
	...
	collid_B->velocity = Point2D(50.0f, 0.0f);
	assert(!Physics::collisionCheck(*collid_A, *collid_B, 1000.0f));

	ObjB->SetPoint(0.0f, 500.0f);
	collid_B->velocity = Point2D(0.0f, -50.0f); //Y-
	assert(!Physics::collisionCheck(*collid_A, *collid_B, 1000.0f));
	...
	ObjB->SetPoint(500.0f, 0.0f);
	collid_B->velocity = Point2D(50.0f, 0.0f); //X+
	assert(!Physics::collisionCheck(*collid_A, *collid_B, 1000.0f));
	...
	collid_B->velocity = Point2D(-100.0f, 0.0f); //X-
	assert(Physics::collisionCheck(*collid_A, *collid_B, 1000.0f));

	ObjB->SetPoint(0.0f, -100.0f);
	collid_B->velocity = Point2D(0.0f, 100.0f); //Y+
	assert(Physics::collisionCheck(*collid_A, *collid_B, 1000.0f));

	ObjB->SetPoint(-100.0f, -100.0f);
	collid_B->velocity = Point2D(100.0f, 100.0f); //X+, Y+
	assert(Physics::collisionCheck(*collid_A, *collid_B, 1000.0f));
...
int WINAPI wWinMain(HINSTANCE i_hInstance, HINSTANCE i_hPrevInstance, LPWSTR i_lpCmdLine, int i_nCmdShow)
{
	...
	//CollisionUnitTest();
	{
		...
		charArray.push_back("data\\LeftWall.json");
		charArray.push_back("data\\RightWall.json");
